...
class _PosPageState extends State<PosPage> {
    ...
    try {
      ...
      if (saleId > 0) {
        ...
        if (mounted) {
          ...
          // عرض الفاتورة كاملة
          if (mounted) {
            _showSaleInvoiceSheet(cartSnapshot, _subtotal, _discountValue, total, saleId: saleId, customerName: selectedCustomer);
          }
  ...
  // ─── عرض الفاتورة في BottomSheet ───
  void _showSaleInvoiceSheet(
          ...
          child: Column(
            ...
            children: [
              Center(
                child: Container(
                  width: 40, height: 4,
                  decoration: BoxDecoration(color: AppTheme.neutralColor, borderRadius: BorderRadius.circular(2)),
                ),
              ),
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  const Text('تفاصيل الفاتورة', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: AppTheme.primaryColor)),
                  if (saleId != null)
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                      decoration: BoxDecoration(color: AppTheme.primaryColor.withOpacity(0.1), borderRadius: BorderRadius.circular(10)),
                      child: Text('R${saleId.toString().padLeft(6, '0')}', style: const TextStyle(fontWeight: FontWeight.bold, color: AppTheme.primaryColor)),
                    ),
                ],
              ),
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      const SizedBox(width: 8),
                      Text('العميل: $customerName', style: const TextStyle(fontWeight: FontWeight.bold, color: AppTheme.errorColor)),
                      const Spacer(),
                      const Text('بيع بالدين', style: TextStyle(fontSize: 11, color: AppTheme.errorColor)),
                    ],
                  ),
                ),
              ],
              ...
              Container(
                ...
                child: const Row(
                  children: [
                    Expanded(flex: 4, child: Text('المادة', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13))),
                    Expanded(flex: 2, child: Text('العدد', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13), textAlign: TextAlign.center)),
                    Expanded(flex: 3, child: Text('السعر', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13), textAlign: TextAlign.center)),
                    Expanded(flex: 3, child: Text('الإجمالي', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13), textAlign: TextAlign.center)),
                  ],
                ),
              ),
                ...
                return Container(
                  ...
                  child: Row(
                    children: [
                      Expanded(
                        flex: 4,
                        child: Text(product.name, style: const TextStyle(fontSize: 13), overflow: TextOverflow.ellipsis),
                      ),
                      Expanded(
                        flex: 2,
                        child: Text('x$qty', style: const TextStyle(fontSize: 13), textAlign: TextAlign.center),
                      ),
                      Expanded(
                        flex: 3,
                        child: Text('$itemPrice', style: const TextStyle(fontSize: 13), textAlign: TextAlign.center),
                      ),
                      Expanded(
                        flex: 3,
                        child: Text('$rowTotal', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
                      ),
                    ],
                  ),
                );
              ...
              if (discountValue > 0) ...[
                Row(
                  ...
                  children: [
                    ...
                    Text('$subtotal $currency', style: const TextStyle(fontSize: 13)),
                  ],
                ),
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    const Text('الخصم', style: TextStyle(fontSize: 13, color: AppTheme.errorColor)),
                    Text('- $discountValue $currency', style: const TextStyle(fontSize: 13, color: AppTheme.errorColor)),
                  ],
                ),
