...
class _BarcodePrintDialogState extends State<_BarcodePrintDialog> {
  ...
  Future<void> _printBarcode() async {
    ...
    final labelW =
        (double.tryParse(settings['barcode_label_width_mm'] ?? '40') ?? 40.0) *
        PdfPageFormat.mm;
    final labelH =
        (double.tryParse(settings['barcode_label_height_mm'] ?? '25') ?? 25.0) *
        PdfPageFormat.mm;
    final padTop =
        (double.tryParse(settings['barcode_padding_top_mm'] ?? settings['barcode_inner_padding_mm'] ?? '2') ?? 2.0) *
        PdfPageFormat.mm;
    final padBottom =
        (double.tryParse(settings['barcode_padding_bottom_mm'] ?? settings['barcode_inner_padding_mm'] ?? '2') ?? 2.0) *
        PdfPageFormat.mm;
    final padLeft =
        (double.tryParse(settings['barcode_padding_left_mm'] ?? settings['barcode_inner_padding_mm'] ?? '2') ?? 2.0) *
        PdfPageFormat.mm;
    final padRight =
        (double.tryParse(settings['barcode_padding_right_mm'] ?? settings['barcode_inner_padding_mm'] ?? '2') ?? 2.0) *
        PdfPageFormat.mm;
    final savedPrinterUrl =
        settings['barcode_printer_url'] ??
    ...
    Future<Uint8List> buildPdf(PdfPageFormat format) async {
      // نأخذ الحجم الفعلي من الطابعة إذا متاح، وإلا نستخدم حجم البطاقة
      final useFormat =
          (format.width > 0 &&
              format.height > 0 &&
    ...
    // محاولة الطباعة المباشرة إذا الطابعة محفوظة
    if (savedPrinterUrl.isNotEmpty) {
      try {
        final directPdf = await buildPdf(labelFormat);
        final ok = await Printing.directPrintPdf(
          printer: Printer(url: savedPrinterUrl, name: savedPrinterName),
          onLayout: (_) async => directPdf,
        );
        if (ok) return;
      } catch (_) {}
    }
    // فتح نافذة الطباعة إذا ما اشتغلت المباشرة
    await Printing.layoutPdf(
      onLayout: buildPdf,
      format: labelFormat,
      name: 'Barcode_${widget.barcode}',
    );
  }
